...
@hydra.main(config_path="conf", version_base=None, config_name="synthesize")
@torch.inference_mode()
def main(cfg):
    out_dir = Path(cfg.out_dir)
    [(out_dir / s).mkdir(parents=True, exist_ok=True) for s in ["wav"]]
    wav_dir = Path(cfg.path.wav_dir)
    f0_dir = Path(cfg.path.f0_dir)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    lit_module = (
        instantiate(cfg.lit_module, params=cfg, _recursive_=False)
        .load_from_checkpoint(cfg.ckpt_path, params=cfg)
        .to(device)
        .eval()
    )
    ...
    for bname in tqdm(bnames, total=len(bnames)):
        ...
        wav = wav.to(device)
        mel = to_mel(wav)
        f0 = torch.FloatTensor(np.load(f0_dir / f"{bname}.npy"))[None, :].to(device)
        f0 = f0[..., : mel.shape[-1]]
